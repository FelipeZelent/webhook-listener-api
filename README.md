...
Na versao atual, ele recebe eventos GitHub em `POST /webhooks/github`, valida o header `X-Signature` com HMAC SHA-256 usando o body bruto da requisicao, impede duplicidade pelo `externalEventId`, salva o evento no PostgreSQL e permite consultar os eventos recebidos em `GET /webhooks/events`.

## Fluxo do webhook

1. O emissor envia um `POST` para `/webhooks/github`
2. A API valida o header `X-Signature`
